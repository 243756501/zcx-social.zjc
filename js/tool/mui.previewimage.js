(function($, window) {
	...
	var PreviewImage = function(options) {
		this.options = $.extend(true, {
			...
			zoom: true,
			header: '<div class="preview-save-btn">保存</div><span class="mui-preview-indicator"></span>',
			footer: ''
		}, options || {});
		this.init();
		this.initEvent();
	...
	proto.init = function() {
		...
		this.element = el;
		this.saveBtn = this.element.querySelector('.preview-save-btn');//自己添加
	...
	proto.initEvent = function() {
		...
		/*自己写的保存图片功能*/
		this.saveBtn.addEventListener('tap',function(e){
            var imageName = actImgObj.src.substring(actImgObj.src.lastIndexOf("/") + 1);
			var imgTask = plus.downloader.createDownload(actImgObj.src,{});
			imgTask.addEventListener( "statechanged", function(task,status){
		    	if(!imgTask){return;}
		    	switch(task.state) {
		    		case 1: // 开始
		    		break;
		    		...
		    		plus.gallery.save(task.filename, function () {
		    			plus.nativeUI.toast('保存相册成功',{duration:"long"});
					},function(){
						mui.toast('图片保存失败');
					});
		    		break;
	...
	proto._loadItem = function(index, isOpening) { //TODO 暂时仅支持img
		...
		actImgObj.index = index;
		actImgObj.src = itemData.lazyload||imgEl.src;
